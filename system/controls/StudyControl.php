...
class StudyControl extends Control
{
    ...
    public function Add($args)
    {
        ...
        if (isset($args['new_patient']))
        {
			...
			$patient->institution = strtoupper($args['institution']);
            
            $res = $dbController->Add($patient);
            if (!$res->rowsAffected)
            {
                $return['error_msg'] = 'The Patient ID you entered conflicts with existing Patient,  Please confirm and re-enter';
                return json_encode($return);
            }
		...
		$study->received = date('Y-m-d H:i:s');
