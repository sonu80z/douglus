...
class Downloads
{
	public function logBurnCDActivity ($args)
	{
		...
		$data = @date("m/d/Y H:i:s") ."OUTPUT:".$args['output']."ERROR:".$args['error']."EXITCODE:".$args['exiterror']."\r\n\r\n\r\n";
		fwrite($f, $data);
		fclose($f);
	}
...
//		$file = 'C:/rentacoder/dpotter/CDBurner/Release/test4.exe';
		...
		$basePath 	= $this->appendSlashes2url('https://'.$_SERVER['HTTP_HOST'].'/bin/', $keyUrlPath);
