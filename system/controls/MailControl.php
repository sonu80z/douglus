...
//         $ok = $this->mail_attachment($files, $mailTo, $my_mail, $my_name, $my_replyto, $my_subject, $my_message);
	//echo 'LINE '.__LINE__.' : <br />'; 
        $ok = $this->mail_send_using_php_mailer($files, $mailTo, $my_mail, $my_name, $my_replyto, $my_subject, $my_message);
	//echo 'LINE '.__LINE__.' : <br />';  die;
    ...
    function mail_send_using_php_mailer($filePath, $mailto, $from_mail, $from_name, $replyto, $subject, $message)
    {
...
//      $body             = file_get_contents('contents.html');
        $body             = $message; //file_get_contents('contents.html');
        ...
        $mail->Username      = "reports@mdidemos.com"; // SMTP account username
        $mail->Password      = "BeProAct2020$";        // SMTP account password
        
        $mail->SetFrom($from_mail, $from_name) ;// 'invoice@techcarexray.com', 'TechcareXray Support');
        $mail->AddReplyTo($replyto); //'invoice@techcarexray.com', 'TechcareXray Support');
...
//      $mailto = "designtop1888@gmail.com";
        $mail->AddAddress($mailto, "Recipient");
        
        if (!is_array($filePath)) $filePath = [$filePath];

        foreach ($filePath as $real_file_path)
        {

            $mail->AddAttachment($real_file_path);      // attachment
        }
