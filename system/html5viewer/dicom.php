    ...
    function returnXML() {
     ...
     function returnObject() {
        ...
        foreach ($this->attrs as $key => $attr) {
            ...
            if (strlen($value) && !($group & 1) && !($name=="Unknown")) $returnObject[str_replace("'", "", str_replace(" ", "", $name))] = htmlspecialchars($value);
        }
        return $returnObject;
    }
    function returnJSON() {
        ...
        foreach ($this->attrs as $key => $attr) {
            ...
            $value = trim($value);
            if (strlen($value) && !($group & 1) && !($name=="Unknown")) $resultJSON[] = '"' . $name . '":"' . htmlspecialchars($value) . '"';

        }
        return "{". implode(",", $resultJSON) . "}";
    }
}
