Ext.define('MdiApp.view.main.StudyGrid',function(){
	var performAjaxAction=function(action, grid, params)
			{
				//base options
	...
	constructor : function(config){
		...
		this.performAjaxAction = function(action,success,failure,params){
		var grid=self;
		if(grid.getSelectionModel().getCount()){
			selected=grid.getSelection();
			if(Ext.isString(action)){
				action={params:{actions:action}}
				
			}
			action.success=function(result){
				if(Ext.isFunction(success)){
					success(result,grid,selected)
				}
			},
			...
			performAjaxAction(action,grid,params)
		}else{
...
//                    pageSize: 15,
	...
	rowcontextmenu:function( grid, record, tr, rowIndex, e, eOpts){
		...
		var self=this
		try{
			var menu=Ext.create('MdiApp.view.main.GridContextMenu',{
				record:record,
				grid:self
			});
			//grid.mask()
