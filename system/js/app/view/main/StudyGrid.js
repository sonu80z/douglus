...
//                                  {header: "Images", dataIndex: 'images'},
                                    ...
                                    {header: '<img src="system/ico/note.png" title="Notes" height="16px" width="16px"/>',sortable:false,menuDisabled:true, width:30, dataIndex: 'note_text', renderer: 
                                        function (value, metaData, record, rowIndex, colIndex, store) 
                                        {
                                            if (value != '')
                                            {
                                                return '<div style="height:16px;width:16px;cursor:pointer;cursor:hand;" class="ico-note" title="' + record.data.note_user + ' (' + record.data.note_date + ') :   ' + record.data.note_text + '"/>';
                                            }
                                            return '';
                                        }},
...
//                    pageSize: 15,
		...
		var listeners={
			...
			itemdblclick:function(){MdiApp.study.viewDicom(self)},
			cellclick:function(view,td,cellIndex,r,tr,rowIndex,e,eOpts){
				if(e.getTarget('.ico-note')){
					self.setSelection(r)
					MdiApp.study.showNoteWindow(self)
				}
			}
